  ...
  (with-sheet-medium (medium sheet)
    (let* ((text-cursor (stream-text-cursor sheet))
      ...
      (flet ((draw-line (line)
        ...
        (multiple-value-bind (cursor-x cursor-y) (cursor-position text-cursor)
          (unless (region-contains-position-p
                   (sheet-visible-region sheet) (1+ cursor-x) (1+ cursor-y))
            (scroll-extent sheet (1+ cursor-x) (1+ cursor-y))))))))
